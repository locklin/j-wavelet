	  ...
	  double *Xout)
{
  ...
  for(t = 0; t < M; t++) {
    ...
    i = 1;
    j = 0;
    /* printf("outer m=%d n=%d i=%d j=%d u=%d\n",m,n,i,j,u); */
	...
	i += 2;
	j += 2;
	/* printf("inner m=%d n=%d i=%d j=%d u=%d\n",m,n,i,j,u); */
	Xout[m] += h[i] * Win[u] + g[i] * Vin[u];
	Xout[n] += h[j] * Win[u] + g[j] * Vin[u];
      }
    }
	   ...
	   double *Wout, double *Vout)
{
  ...
  for(t = 0; t < N; t++) {
    k = t;
    printf("outer k=%d t=%d\n",k,t);
    Wout[t] = ht[0] * Vin[k];
    Vout[t] = gt[0] * Vin[k];
    for(n = 1; n < L; n++) {
      k -= (int) pow(2.0, (double) j - 1.0);
      if(k < 0) k += N;
      printf("inner k=%d t=%d\n",k,t);
      Wout[t] += ht[n] * Vin[k];
      Vout[t] += gt[n] * Vin[k];
    }
  }
